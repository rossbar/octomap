...
namespace octomap {
  ...
  ConeOcTreeDrawer::ConeOcTreeDrawer() 
    : OcTreeDrawer() {
  }
  ...
  // Based on OcTreeDrawer::setCubeColorHeightmap w/ slight modification
  unsigned int ConeOcTreeDrawer::setCubeColorCVP(double cvp,
               const unsigned int &current_array_idx, GLfloat** glColorArray)
  {
    if (glColorArray == NULL) return current_array_idx;

    unsigned int colorIdx = current_array_idx;
    // color for all 4 vertices (same cvp)
    for (int k = 0; k < 4; ++k)
    {
      if (m_colorMode == CM_GRAY_HEIGHT)
        SceneObject::heightMapGray(cvp, *glColorArray + colorIdx); // sets rgb
      else
        SceneObject::heightMapColor(cvp, *glColorArray + colorIdx); // sets rgb
      // set Alpha value
  ...
  void ConeOcTreeDrawer::setOcTree(const AbstractOcTree& tree_pnt,
                                    const octomap::pose6d& origin_,
                                    int map_id_) {

    const OcTreeCone& tree = ((const OcTreeCone&) tree_pnt);
    ...
    for(OcTreeCone::tree_iterator it = tree.begin_tree(this->m_max_tree_depth),
...
//                                                         color_idx_occupied_thres, &m_occupiedThresColorArray);
            color_idx_occupied_thres = setCubeColorCVP(it->getConeVoxelProbability(),
...
//                                                  color_idx_occupied, &m_occupiedColorArray);
            color_idx_occupied = setCubeColorCVP(it->getConeVoxelProbability(),
                                                 color_idx_occupied,
